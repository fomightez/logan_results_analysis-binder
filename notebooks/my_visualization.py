...
def plot_homo_sapiens_counts(dfs, labels=None, palette='husl', order=None, legend_text=None):
    ...
    # Customize the plot
    plt.title(('Number of $\mathit{Homo\ sapiens}$ Transcriptomic SRA Entries\nContaining the '
...
def plot_homo_sapiens_counts(dfs, labels=None, palette=None, order=None, gene_name=gene_name):
    ...
    ax.set_ylabel('Count')
    ax.set_title(('Number of $\mathit{Homo\ sapiens}$ Transcriptomic SRA Entries Thru Dec. 2023\nContaining '
...
# Example usage:
labels = [simplifytags(x) for x in file_tags_per_file]
desired_order = [labels[2],labels[1],labels[3], labels[0]]  # Replace with your actual k-mer names in desired order
colors = ['#1f77b4', 'cornflowerblue', '#ff7f0e', '#ef6f2e']
